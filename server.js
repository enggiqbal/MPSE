...
app.post('/run', multipartMiddleware, function (req, res) {
    ...
    if (req.body.data == "preloaded") {
        ...
        if (preloadeddata == 'circlesquire')
            datapath = ['-d', 'MPSE-web/MPSE/datasets/dataset_3D/sq_cir_tr_dataset/350/data_mat_cir_350.csv', 'MPSE-web/MPSE/datasets/dataset_3D/sq_cir_tr_dataset/350/data_mat_sq_350.csv', 'MPSE-web/MPSE/datasets/dataset_3D/sq_cir_tr_dataset/350/data_mat_tr_350.csv']
    }
    ...
    var parameters = ['mpse.py', '-n', sample_size, '-e', req.body.EXPERIMENT_NAME, '-ps', req.body.projection_set, '-t', projections_type, '-p',req.body.projections, '-max_iters', req.body.max_iters];
