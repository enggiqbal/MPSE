...
list=pd.read_csv( 'proj_joblist.csv', header=None)
f=open("proj_hpc_multiview_job_template.pbs")
pbs=f.read()
...
for i in range(0, list.shape[0]):
    ...
    print(data_path)
    pbs_job=open("proj_pbs/"+job_name+".pbs", "w")
    ...
    pbs_job.close()
    runtext=runtext+ " qsub proj_pbs/" + job_name+".pbs \n"

r=open("proj_hpc_jobs_run.txt", "w")
r.write(runtext)
r.close()
