...
def draw2d(points,outdir,js_file_name,proj, type):
    filename=outdir+"cluster_"+js_file_name.replace(".js", proj) + ".png"
    fig = plt.figure()
    ax = plt.axes()
    ax.grid(False)
    plt.axis('off')
    l=pd.read_csv("/Users/iqbal/multiview3d/dataset_3D/clusters_dataset/labels.csv",  header=None)
    colors = ['red','green']
    markers=['s','o']
    f_style=['full','none' ]
    c=[colors[x%2] for x in l[0]]
    m=[markers[x%2] for x in l[1]]
    f=[f_style[x%2] for x in l[2]]
    #print(f)
    #import pdb; pdb.set_trace()
    #ax.scatter(points.T[0], points.T[1],  c='green', marker="x" ,  cmap='Greens');
    #ax.scatter(points.T[0], points.T[1],  c=c, marker=m ,  cmap='Greens');
    for i in range(0, len(points)):
        ax.plot(points[i][0], points[i][1],  c=c[i], marker=m[i] ,fillstyle=f[i]);
    #plt.show()
    plt.savefig(filename)
